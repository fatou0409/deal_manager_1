...
export default function PipeForm() {
  ...
  const submit = async (e) => {
    ...
    // ✅ Payload pour la table Pipe (pas Deal)
    const payload = {
      client: f.client.trim(),
      commercial: f.ic || user?.name || user?.email || "",
      secteur: f.secteur,
      projet: f.projets?.trim() || "(Projet à préciser)",
      ca: Number(f.budget || 0),
      statut: "Open",
      semestre: state?.selectedSemestre || "2025-S1",
      dateCreation: todayStr(),
    };

    try {
      console.log("📤 Envoi du pipe:", payload);
      
      // ✅ CORRIGÉ : Appelle /pipe au lieu de /deals
      const saved = await api.post("/pipe", payload);
      
      console.log("✅ Pipe créé:", saved);
      ...
      <div className="relative overflow-hidden rounded-3xl border border-black/10 bg-gradient-to-tr from-orange-500 to-black text-white shadow-2xl">
        ...
        <div className="relative p-6 md:p-8">
          ...
          <p className="mt-1 text-white/80 text-sm">
            Renseigne l'opportunité en cours (budget estimatif).
          </p>
        </div>
      </div>
      ...
      <form onSubmit={submit} className="space-y-6">
        ...
        <div className="flex justify-end gap-2">
          ...
          <button
            type="button"
            onClick={() =>
              setF({ client: "", ic: "", secteur: "", projets: "", budget: "" })
            }
